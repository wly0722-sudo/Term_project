...
DAVIS_ROOT = "./DAVIS"
SEQUENCE = "bear"
OUTPUT_VIDEO_PATH = "./opencv_baseline_visualization.mp4"
...
def run_opencv_pipeline():
    ...
    # 1. 첫 프레임 로드 및 트래커 초기화
    first_frame = cv2.imread(img_files[0])
    frame_h, frame_w, _ = first_frame.shape
    gt_box = mask_to_bbox(mask_files[0])
    
    # [버전 에러 해결] OpenCV 버전에 따라 자동으로 알맞은 트래커 생성
    try:
        tracker = cv2.TrackerCSRT_create()
    except AttributeError:
        tracker = cv2.legacy.TrackerCSRT_create()
        
    tracker.init(first_frame, tuple(gt_box))

    # 2. 비디오 인코더 설정 (시각화 박스를 다 보여주기 위해 원본 16:9 크기인 frame_w, frame_h 지정)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out_video = cv2.VideoWriter(OUTPUT_VIDEO_PATH, fourcc, 24.0, (frame_w, frame_h))

    print("🚀 OpenCV CSRT Baseline 시각화 파이프라인 시작...")
    iou_list, fps_list = [], []
    ...
    for i in range(1, len(img_files)):
        ...
        success, box_pred = tracker.update(frame)
        fps = 1.0 / (time.time() - start_time)
        box_pred = list(box_pred) if success else [0, 0, 0, 0]
        ...
        if success:
            ...
            cv2.putText(frame, "OpenCV CSRT", (px, py - 10), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

        # C. 좌측 상단 실시간 스코어 보드 (검은색 배경 투명 박스 효과)
    ...
    out_video.release()
    print(f"✅ OpenCV 시각화 비디오 저장 완료! -> {OUTPUT_VIDEO_PATH}")
