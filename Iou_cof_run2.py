...
# 💡 [변인 통제] Run2 (AdamW + Cosine) 학습 완료 가중치 파일 하나만 고정 사용합니다.
...
def visualize_ablation_run2(model_path, seq_name, output_filename, exp_label, conf_thresh, iou_thresh):
    """Run2 모델 가중치 고정 상태에서 인퍼런스 파라미터 세팅만 다르게 하여 테스트"""
...
def main():
    if not os.path.exists(RUN2_WEIGHTS):
        print(f"❌ Run2 가중치 파일을 찾을 수 없습니다. 경로를 확인하세요: {RUN2_WEIGHTS}")
        return
    ...
    # 🕵️‍♂️ 실험 A: Run2 가중치 + Strict 세팅
    print(f"🏃‍♂️ [실험 A - Run2] Strict Setting (Conf 0.50 / NMS IoU 0.30) 연산 중...")
    out_a = f"./ablation_run2_strict_{selected_seq}.mp4"
    iou_a, fps_a = visualize_ablation_run2(RUN2_WEIGHTS, selected_seq, out_a, "Run2_Strict (Conf:0.5)", conf_thresh=0.5, iou_thresh=0.3)
    print(f"📊 [실험 A - Run2 결과] 평균 IoU: {iou_a:.4f} | 평균 FPS: {fps_a:.2f}\n")

    # 🕵️‍♂️ 실험 B: Run2 가중치 + Flexible 세팅
    print(f"🏃‍♂️ [실험 B - Run2] Flexible Setting (Conf 0.25 / NMS IoU 0.60) 연산 중...")
    out_b = f"./ablation_run2_flexible_{selected_seq}.mp4"
    iou_b, fps_b = visualize_ablation_run2(RUN2_WEIGHTS, selected_seq, out_b, "Run2_Flexible (Conf:0.25)", conf_thresh=0.25, iou_thresh=0.6)
    print(f"📊 [실험 B - Run2 결과] 평균 IoU: {iou_b:.4f} | 평균 FPS: {fps_b:.2f}\n")
    
    print("="*50)
    print("🎉 Run2 가중치 기반 인퍼런스 파라미터 소거 연구(Ablation Study)가 완료되었습니다.")
